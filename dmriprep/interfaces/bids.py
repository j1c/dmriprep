...
class BIDSDataGrabberOutputSpec(TraitedSpec):
    ...
    dwi = OutputMultiPath(desc='output diffusion images')
...
class BIDSDataGrabber(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        ...
        if self._require_dwis and not bids_dict['dwi']:
            raise FileNotFoundError('No diffusion images found for subject sub-{}'.format(
                self.inputs.subject_id))

        for imtype in ['dwi', 't2w', 'fmap', 'roi']:
...
class DerivativesDataSinkInputSpec(BaseInterfaceInputSpec):
    ...
    source_file = File(exists=False, mandatory=True, desc='the input dwi file')
